...
selected_df["Duration"] = round((selected_df["Duration"] / np.timedelta64(1, "m")))

# subset of total time and number of films per category
selected_duration = selected_df.loc[:, ("Submission Categories", "Duration")]

# new dataframe without necceary columns for export
selected_excel_export = selected_df.drop(
    columns=[
        "Submission Status",
        "Submission ID",
        "Birthdate",
...
selected_excel_export.sort_values(by="Submission Categories").to_excel(
    DATA_FOLDER / "OCFF-2021_selected-films_01-14-21.xlsx"
)
